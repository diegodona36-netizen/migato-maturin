...
class AppController {
    ...
    try {
      ...
      if (circuitosContainer) {
        ...
        circuitos.forEach(c => {
          ...
          div.className = `p-2.5 rounded-xl border text-sm flex items-center justify-between transition ${bg}`;
          div.innerHTML = `
            <div>
              <p class="font-bold text-slate-900 leading-tight">${c.nombre}</p>
              <p class="text-sm text-slate-500">${c.subestacion} • Voltaje: ${c.voltaje}V</p>
            </div>
            <div class="text-right shrink-0">
              <span class="font-mono font-bold text-sm ${color}">${c.disponibilidadPct}%</span>
              <span class="block text-sm text-slate-400 font-semibold">${c.fallas24h > 0 ? `${c.fallas24h} falla(s)` : "Estable"}</span>
            </div>
          `;
          ...
          monagasEvents.forEach(ev => {
            ...
            tr.innerHTML = `
              ...
              <td class="px-4 py-3">
                ...
                <div class="text-sm text-emerald-700 font-mono">✓ ${ev.patron}</div>
              </td>
            `;
  ...
  renderStateDetailPanel() {
    ...
    if (s.eventos && s.eventos.length > 0) {
      s.eventos.forEach(ev => {
        eventsHtml += `
          <div class="p-3 rounded-xl bg-red-50/60 border border-red-100 text-sm space-y-1.5">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between border-b pb-3">
          <div class="flex items-center gap-2">
            ...
            <span class="px-2 py-0.5 rounded text-sm font-bold bg-red-100 text-red-700">${s.tier}</span>
          </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="p-3 rounded-xl bg-slate-50 border border-slate-200">
            <p class="text-sm font-semibold text-slate-500 flex items-center gap-1">
          ...
          <div class="p-3 rounded-xl bg-slate-50 border border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-0.5 leading-tight">
        ...
        <div class="space-y-2.5 text-sm font-medium pt-1">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="font-mono font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
              ${s.metrics.packetLossPct}% <span class="text-sm font-normal text-slate-400">(normal &lt;10%)</span>
            </span>
          </div>
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="font-mono font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
              ${s.metrics.latenciaMs}ms <span class="text-sm font-normal text-slate-400">(base ${s.metrics.baseLatency}ms)</span>
            </span>
          </div>
  ...
  renderStateRanking() {
    ...
    this.currentData.estados.slice(0, 10).forEach((s, idx) => {
      ...
      tr.innerHTML = `
        <td class="px-3 py-2 text-slate-400 font-mono text-sm">${idx + 1}</td>
        <td class="px-3 py-2 text-slate-900 font-semibold flex items-center gap-1.5">
          <span class="w-2 h-2 rounded-full" style="background-color: ${color}"></span>
          <span>${s.nombre}</span>
          <span class="text-sm text-red-600 font-bold">${s.tier}</span>
        </td>
        <td class="px-3 py-2 text-right font-mono font-bold" style="color: ${color}">
          ${s.electricidadPct}%
        </td>
        <td class="px-3 py-2 text-right text-slate-500 font-mono text-sm">
  ...
  renderCortesElectricosTab() {
    ...
    this.currentData.estados.forEach(s => {
      ...
      if (hasEvents) {
        s.eventos.forEach(ev => {
          innerDetails += `
            <div class="p-3 my-2 rounded-xl bg-red-50/50 border border-red-100 text-sm space-y-1 ml-4 sm:ml-8">
              <p class="font-bold text-slate-900">${ev.detalle}</p>
              <p class="text-slate-600 font-mono text-sm">${ev.fecha} • -${ev.caidaPct}% • ${ev.duracion} • <span class="text-rose-600">📍 ${ev.tipo}</span></p>
              <p class="text-emerald-700 text-sm">✓ ${ev.patron}</p>
            </div>
          `;
        });
      }
      ...
      stateDiv.innerHTML = `
        <div class="flex items-center justify-between p-3.5 hover:bg-slate-50 cursor-pointer transition select-none corte-accordion-header">
          <div class="flex items-center gap-2">
            <span class="text-slate-400 text-sm transform transition-transform chevron-icon ${hasEvents ? "rotate-90" : ""}">▸</span>
          ...
          <div class="flex items-center gap-4 text-sm">
            <span class="px-2 py-0.5 rounded font-bold border text-sm ${confBadgeColor}">${s.confianza}</span>
  ...
  renderEventosTab() {
    ...
    if (filterPillsContainer) {
      ...
      distinctStates.forEach(st => {
        ...
        btn.className = `px-3 py-1 text-sm font-semibold rounded-full transition ${isActive ? "bg-sky-600 text-white shadow-sm font-bold" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`;
        btn.textContent = st;
    ...
    events.forEach(ev => {
      ...
      tr.innerHTML = `
        ...
        <td class="px-4 py-3"><span class="px-2 py-0.5 rounded font-mono font-bold text-sm border ${fuenteBadge}">${ev.fuente}</span></td>
        <td class="px-4 py-3 ${sevColor}">${ev.severidad}</td>
        <td class="px-4 py-3 font-mono font-black text-right ${sevColor}">
          <div>${ev.score}</div>
          <div class="text-sm text-slate-400 font-normal">${ev.duracion}</div>
        </td>
      `;
      tableBody.appendChild(tr);
    });
