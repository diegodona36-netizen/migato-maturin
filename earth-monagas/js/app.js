 ...
 * Individualización Territorial por Parroquia y Cero Desborde Móvil
 */
...
class EarthMonagasApp {
  ...
  init() {
    ...
    // Cargar parroquia inicial individualizada
  ...
  selectParish(munId, parishId) {
    ...
    if (navLoc) navLoc.textContent = `${munObj.nombre} > ${parish.nombre}`;

    // Actualizar título de la pestaña con la parroquia individual
    document.title = `${parish.nombre} (${munObj.nombre}) • Google Earth Pro`;

    // Mostrar perímetro y elementos de esta parroquia únicamente
  ...
  /**
   * Renderiza el Árbol de Lugares de manera INDIVIDUALIZADA para la parroquia activa.
   * Evita tener las 44 parroquias amontonadas.
   */
  renderPlacesTree(filterQuery = "") {
    ...
    let html = `
      <!-- Tarjeta de Parroquia Individual Activa -->
      <div class="bg-slate-950/80 p-2.5 rounded-xl border border-sky-500/30 mb-2 shadow-inner">
        <div class="flex items-center justify-between mb-1">
          <span class="text-[10px] font-bold text-sky-400 uppercase tracking-wider">${munObj?.nombre || 'Municipio'}</span>
          <span class="text-[10px] font-mono px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/30">Activa</span>
        </div>
        <h4 class="text-sm font-black text-white truncate">${pData.nombre}</h4>
        <div class="flex items-center gap-3 mt-1.5 text-[11px] font-mono text-slate-400">
          <span>⬡ ${pData.poligonos?.length || 0} sectores</span>
          <span>〰️ ${pData.rutas?.length || 0} vías</span>
        </div>
      </div>
      ...
      <div class="mb-3">
        <div class="flex items-center justify-between px-2 py-1 text-slate-400 font-bold text-[11px] uppercase tracking-wider">
          <span class="flex items-center gap-1.5 text-sky-300">
            ...
            <span>Sectores (${filteredPolys.length})</span>
          </span>
        </div>
    ...
    if (filteredPolys.length === 0) {
      html += `
        <div class="text-[11px] text-slate-500 px-3 py-2 italic bg-slate-950/40 rounded-lg border border-slate-800/40">
          No hay polígonos trazados. Toca <strong>Polígono</strong> arriba para marcar el primer sector.
        </div>
      `;
    } else {
      filteredPolys.forEach(poly => {
        html += `
          <div class="flex items-center justify-between py-1.5 px-2 bg-slate-950/60 hover:bg-slate-800/80 rounded-lg group text-xs border border-slate-800/50 transition">
            ...
            <button onclick="window.earthApp.deleteItem('${this.selectedMunId}', '${this.selectedParishId}', 'poligono', '${poly.id}')" class="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 p-1 transition" title="Eliminar sector">
    ...
    if (filteredRoutes.length === 0) {
      html += `
        <div class="text-[11px] text-slate-500 px-3 py-2 italic bg-slate-950/40 rounded-lg border border-slate-800/40">
          No hay calles trazadas. Toca <strong>Ruta</strong> arriba para trazar una calle.
        </div>
      `;
    } else {
      filteredRoutes.forEach(r => {
        html += `
          <div class="flex items-center justify-between py-1.5 px-2 bg-slate-950/60 hover:bg-slate-800/80 rounded-lg group text-xs border border-slate-800/50 transition">
            ...
            <button onclick="window.earthApp.deleteItem('${this.selectedMunId}', '${this.selectedParishId}', 'ruta', '${r.id}')" class="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 p-1 transition" title="Eliminar calle">
              <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
            </button>
          </div>
        `;
  ...
  renderParishesCatalog() {
    ...
    catalog.innerHTML = CATALOGO_MONAGAS.map(mun => {
      ...
      const parishesButtons = mun.parroquias.map(p => {
        ...
        return `
          <button onclick="window.earthApp.selectParishFromModal('${mun.id}', '${p.id}')" class="px-2.5 py-1.5 rounded-lg text-left text-xs font-semibold transition flex items-center justify-between ${isCurrent ? 'bg-sky-600 text-white font-black shadow' : 'bg-slate-800 hover:bg-slate-700 text-slate-200'}">
            <span>${p.nombre}</span>
  ...
  setupToolbarEvents() {
    ...
    // 4. Exportar KML nativo (Individual de la Parroquia Activa)
    ...
    const btnExportDesktop = document.getElementById("btn-export-kml");
    if (btnExportDesktop) btnExportDesktop.addEventListener("click", handleExportKml);

    const btnExportMobile = document.getElementById("btn-mobile-export-kml");
    if (btnExportMobile) btnExportMobile.addEventListener("click", () => {
      handleExportKml();
      this.closeMobileDropdown();
    });

    // 5. Brújula / Reset Norte
    const handleCompass = () => {
    ...
    if (btnCompass) btnCompass.addEventListener("click", handleCompass);

    const btnCompassMobile = document.getElementById("btn-mobile-compass");
    if (btnCompassMobile) btnCompassMobile.addEventListener("click", () => {
      handleCompass();
      this.closeMobileDropdown();
    });

    // 6. Menú Desplegable "⋯ Más" en móvil
    const btnMobileMore = document.getElementById("btn-mobile-more-tools");
    const dropdownMobile = document.getElementById("dropdown-mobile-tools");

    if (btnMobileMore && dropdownMobile) {
  ...
  setupOverlayModal() {
    ...
    const btnOpenDesktop = document.getElementById("btn-tool-overlay");
    const btnOpenMobile = document.getElementById("btn-mobile-overlay");
    ...
    if (btnOpenDesktop) btnOpenDesktop.addEventListener("click", openOverlay);
    if (btnOpenMobile) btnOpenMobile.addEventListener("click", openOverlay);
