# Reglas y Contexto Permanente del Proyecto: MIGATO Monagas 2026

## 1. Identidad Institucional y Línea de Mando Política
- **Organización:** Movimiento Independiente Ganamos Todos (MIGATO).
- **Líder Supremo y Candidato:** José Gregorio "El Gato" Briceño (Ex-Gobernador histórico del Estado Monagas).
- **Responsable Técnico Regional:** Ing. Diego Donado (Responsable de Ciencia, Tecnología y Ciberdefensa).
- **Ámbito Territorial:** Estado Monagas, Venezuela (13 Municipios, 44 Parroquias, Sede Central en Maturín).
- **Doctrina:** Todo informe, análisis, módulo o propuesta debe reflejar sin excepciones la identidad del partido MIGATO y la conducción de El Gato Briceño.

## 2. Estándar Obligatorio de Informes y Documentos DOCX / PDF
- Todo informe técnico, económico, propuesta institucional o presupuesto debe generarse bajo el **Estándar Oficial IUTIRLA / MIGATO** documentado en `docs/ESTANDAR_FORMATO_DOCUMENTOS_IUTIRLA.md`:
  - **Formato primario:** Microsoft Word (`.docx`) nativo con estilos XML OpenXML y PDF compilado en LibreOffice.
  - **Estructura estricta:** 
    - **Página 1:** Portada Institucional (Cintillo oficial de Venezuela y Monagas, logo central MIGATO de 1.5" x 1.5", bloque de autoría del Ing. Diego Donado, fecha y títulos).
    - **Página 2:** Introducción, Premisas Operativas y Tablas de Costos (encabezado gris `#EAEAEA`, texto 8.5pt negrita, bordes negros superior e inferior).
    - **Página 3:** Doctrina de Vanguardia Cloud VPS + Bóveda Local RAID 1, Recomendación Final y Bloque de Firmas formal (Comando Estratégico MIGATO & Ing. Diego Donado).
  - **Tipografía y Maquetación:** Times New Roman 11.5pt (`sz=23`), interlineado 1.15 (`line=280`), sangría de 1 cm (`567 dxa`) en primera línea de párrafo, encabezado continuo con mini logo y paginación "Página X de Y".
  - **Sincronización Cloud:** Subir obligatoriamente el archivo `.docx` a Google Drive vía MCP con `convert_to_doc=True` para entregar el enlace directo a Google Docs.

## 3. Bóveda de Conocimiento Obsidian y Word Tree
- Las decisiones clave, infraestructura, módulos y acuerdos de campaña se sincronizan en la Bóveda de Obsidian local (`vault/` enlazada a `~/Obsidian/MIGATO-Vault`).
- Utilizar siempre enlaces bidireccionales `[[...]]` para mantener conectado el Grafo de Conocimiento (Word Tree) y permitir consultas relacionales a la IA.
